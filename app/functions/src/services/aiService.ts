...
export const aiService = {
  async suggestTags(contactId: string, tagVocab?: string[]) {
    const bundle = await contactsRepo.getBundle(contactId);
...
- Do not suggest tags that are already assigned.`;
  ...
  async applyTags(contactId: string, tagNames: string[]) {
    const unique = Array.from(new Set(tagNames.map((t) => t.trim()).filter(Boolean)));
    const tagIds: string[] = [];
    for (const name of unique) {
      const t = await tagsRepo.upsert({ name });
      if (t?.id) tagIds.push(t.id);
    }
    await tagsRepo.assign(contactId, { tag_ids: tagIds, assigned_by: "ai", confidence: 0.8 });
    return { applied: true, created_tags: 0, assigned: tagIds.length };
  },

  async searchAi(query: string, limit: number) {
    const candidates = await contactsRepo.searchCandidates(query, Math.min(limit * 3, 300));
